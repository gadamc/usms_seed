...
def extract_individual_results(html, verbose=False):
    soup = BeautifulSoup(html, 'html.parser')
    columns = ["name", "age", "event_name", "club", "seed_time", "final_time",
               "gender", "stroke_type", "distance", "unit"]
...
def process_meet_list_url(meet_list_url):
...
def process_participant_url(participant_url, a_meet_uuid):
...
@click.option(
    ...
    default=-1,
    required=False,
    help="set value to put upper limit on number of meets scraped. -1 means no limit.",
)
def main(**kwargs):
    ...
    page_0 = kwargs['start_page']
    page_n = kwargs['number_of_pages'] + page_0
    meetlist_urls = [meetlist_url_generator(100*page) for page in range(page_0, page_n)]

    with ThreadPoolExecutor(max_workers=kwargs['threads']) as executor:
        meet_list_dfs = list(executor.map(process_meet_list_url, meetlist_urls))

    meet_list_df = pd.concat([df for df in meet_list_dfs if df is not None])
    meet_list_df.to_csv(f'{kwargs["save_dir"]}/{kwargs["output_file_name"]}_list.csv', index=False)
    logger.debug('meet list shape: ' + str(meet_list_df.shape))
    logger.debug(meet_list_df.head())

    meet_results_dfs = []
    max_meets = meet_list_df.shape[0]
    if kwargs['max_meets'] > 0:
        max_meets = kwargs['max_meets']

    for a_meet_url, a_meet_uuid, a_meet_id in meet_list_df[['meet_url', 'meet_uuid', 'meet_id']].values[:max_meets]:
        ...
        if meet_response.status_code == 200:
            ...
            with ThreadPoolExecutor(max_workers=kwargs['threads']) as executor:
                participant_results = list(executor.map(lambda url: process_participant_url(url, a_meet_uuid), participant_urls))

        meet_results_dfs.extend([df for df in participant_results if df is not None])

    meet_results_df = pd.concat(meet_results_dfs)
    meet_results_df.to_csv(f'{kwargs["save_dir"]}/{kwargs["output_file_name"]}_results.csv', index=False)
